...
ais_file = pd.read_csv('/home/richardchen123/Documents/data/dataset-ais-origin/drop_decimals_finish_drop.csv')
...
for group in groupby_day:
    group[1].to_csv('/home/richardchen123/Documents/data/data_resemble/%s-%s.csv' % (str(name), str(group[0])), index=False)
...
for f in trajectory_process:
    ...
    for group in groupby_hour:
        group[1].to_csv('/home/richardchen123/Documents/data/data_resemble/test/groupby_hour/%s-%s.csv' % (filename, str(group[0])), index=False)
